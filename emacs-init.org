...
** Fix up UI
   ...
   #+BEGIN_SRC emacs-lisp :exports code :results silent
     (defun shlomi/highlight-words ()
       (font-lock-add-keywords nil '(("\\<\\(FIXME\\|TODO\\|LEARN\\|BUG\\):" 1 font-lock-warning-face t))))
   #+END_SRC
...
*** Smart-mode-line
    ...
    #+END_SRC
** Folding everywhere
   When files are large and messy (I am talking to you, ugly json), folding could bring back some sense and structure

   #+BEGIN_SRC emacs-lisp :exports code :results silent
     (use-package yafolding
       :ensure t
       :config (progn)
       :bind (("S-<tab>" . yafolding-toggle-element))
     )
   #+END_SRC
** Flyspell popup
   Flyspell forces your eye to move away from editing position to see the possible corrections. Not only that, but
   recently the options are shown at the top, which moves the entire buffer position down. Lets fix that with some popups!
   Lets fyl to the netherlands 
   #+BEGIN_SRC emacs-lisp :exports code :results silent
     (use-package flyspell-popup
       :ensure t
       :config (progn
                 (add-hook 'flyspell-mode-hook #'flyspell-popup-auto-correct-mode))
       :bind (("M-#" . flyspell-popup-correct)))
   #+END_SRC
* Emacs-lisp
...
* Docker
  Lets get some docker-mode going on.
  
  First, lets set up dockerfile-mode. Later maybe I will setup more docker/docker-tramp stuff
  #+BEGIN_SRC emacs-lisp :exports code :results silent
  ...
  #+BEGIN_SRC emacs-lisp :exports code :results silent
    (use-package org
      :config (progn
                ...
                ;; Lets make a day be 8 hours of work, a week be 5 days of work
...
** Clock
   ...
   Unfortunatly, the code here does nothing since it runs before org has loaded which overrides this. 
   Until I find a better way, I also stuck this code at the use-package init function above
   #+BEGIN_SRC emacs-lisp :exports code :results silent
     (setq org-duration-units `(("min" . 1)
                                ("h" . 60)
                                ("d" . ,(* 60 8))
...
** HTML export
   Lets install htmlize so we could export org-mode files as html
   #+BEGIN_SRC emacs-lisp :exports code :results silent
     (use-package htmlize
       :ensure t)
   #+END_SRC

*** HTML Permenant ids for exported titles
    Following [[https://writequit.org/articles/emacs-org-mode-generate-ids.html][this tutorial]], I found this out:
    ...
    #+BEGIN_SRC emacs-lisp :exports code :results silent
      ...
      (defun eos/org-id-new (&optional prefix)
        "Create a new globally unique ID.
         ...
         P REFIX can specify the prefix, the default is given by the variable
        ...
        (let* ((prefix (if (eq prefix 'none)
          ...
          (cond
           ((memq org-id-method '(uuidgen uuid))
            ...
            (setq unique (concat
                          (sanitize (org-entry-get nil "ITEM"))
                          "-"
                          (number-to-string (random 1000))))
            ;;(unless (org-uuidgen-p unique) (setq unique (org-id-uuid)))
            )
...
** Custom block shortcuts
   #+BEGIN_SRC emacs-lisp :exports code :results silent
     (require 'org-tempo)
     (tempo-define-template "elr" ; just some name for the template
           '("#+begin_src emacs-lisp :exports results" n> p > n> "#+end_src")
           "<elr"
           "Insert emacs-lisp with results" ; documentation
           'org-tempo-tags)

     (tempo-define-template
      "elc"
      '("#+begin_src emacs-lisp :exports code :results silent" n> p > n> "#+end_src")
      "<elc"
      "Insert emacs lisp with no results that exports code")

     (tempo-define-template "elr"  '( "#+begin_src emacs-lisp :exports results" n> p > n> "#+end_src") "<elr" "" 'org-tempo-tags)
     (tempo-define-template "elc"  '( "#+begin_src emacs-lisp :exports code :results silent" n> p > n> "#+end_src") "<elc" "" 'org-tempo-tags)
     (tempo-define-template "clj"  '( "#+begin_src clojure" n> p > n> "#+end_src") "<clj" "" 'org-tempo-tags)
     (tempo-define-template "cljc" '( "#+begin_src clojure :exports code :results silent" n> p > n> "#+end_src") "<cljc" "" 'org-tempo-tags)
     (tempo-define-template "shc"  '( "#+begin_src shell :exports code :results silent" n> p > n> "#+end_src") "<shc" "" 'org-tempo-tags)
     (tempo-define-template "sh"   '( "#+begin_src shell" n> p > n> "#+end_src") "<sh" "" 'org-tempo-tags)
     (tempo-define-template "sho"  '( "#+begin_src shell :results output" n> p > n> "#+end_src") "<sho" "" 'org-tempo-tags)
     (tempo-define-template "t"    '( "#+begin_tip" n> p > n> "#+end_tip") "<t" "" 'org-tempo-tags)
     (tempo-define-template "w"    '( "#+begin_warning" n> p > n> "#+end_warning") "<w" "" 'org-tempo-tags)
     (tempo-define-template "n"    '( "#+begin_note" n> p > n> "#+end_note") "<n" "" 'org-tempo-tags)
     (tempo-define-template "f"    '( "#+begin_info" n> p > n> "#+end_info") "<f" "" 'org-tempo-tags)
   #+END_SRC
